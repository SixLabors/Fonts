...
public class BidiCharacterTests
{
    ...
    private bool ICUTestsImpl()
    {
        ...
        for (int lineNumber = 1; lineNumber < lines.Length + 1; lineNumber++)
        {
            ...
            int[] codePoints = [.. fields[0].Split(' ').Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x)).Select(x => Convert.ToInt32(x, 16))];
            ...
            sbyte[] resolvedLevels = [.. fields[3].Split(' ').Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x)).Select(x => x == "x" ? (sbyte)-1 : Convert.ToSByte(x))];

            // Parse field 4 - resolved levels
            int[] resolvedOrder = [.. fields[4].Split(' ').Select(x => x.Trim()).Where(x => !string.IsNullOrEmpty(x)).Select(x => Convert.ToInt32(x))];
